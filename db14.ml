...
and _ rel_kind =
  | NoRel : [`norel] rel_kind
  | OneToMany_One : [`otm_one] rel_kind
  | OneToMany_Many : [`otm_many] rel_kind
...
let vBDs = field ~name:"VBDs" ~cls:VM ~rw:RW ~ty:(List (Refv VBD)) ~fget:(fun vm -> vm.vBDs) ~fset:(fun x vm -> {vm with vBDs=x})
