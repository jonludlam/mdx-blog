...
and ('a, 'b) otm_pair = (('a, 'b Ref.t, [`ro]) field * ('b, 'a Ref.t list, [`ro]) field)
...
let vM = field ~name:"vM" ~cls:VBD ~ty:(Refv VM) ~rw:RO ~fget:(fun vbd -> vbd.vM) ~fset:(fun x vbd -> {vM=x})
...
let setrel : 'a Ref.t -> ('a, 'b) otm_pair -> 'b Ref.t -> unit = fun refo (fieldo, fieldm) refm ->
...
let set : 'a Ref.t -> ('a,'b,[`rw]) field -> 'b -> unit = fun ref field x ->
