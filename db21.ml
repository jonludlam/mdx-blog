...
and 'a ref = Ref : ('a RefMap.t, 'a option, [`rw]) field -> 'a ref | NullRef : 'a cls -> 'a ref
...
let rec string_of_typ : type a. prcls -> a typ -> string = fun {pr} t ->
  match t with
  | String -> "string"
  | Int -> "int"
  | Refv cls -> Printf.sprintf "refv(%s)" (pr cls)
...
let cls_of_ref = function | Ref x -> (match x.f_cls with | RefMap x -> x | _ -> failwith "Unknown cls") | NullRef cls -> cls
...
module VM = struct
  ...
  let vm_record = let fields = [B uuid; B name_label; B vBDs; B other_config] in {r_fields = (fun _ -> fields); r_empty = empty_vm}
  let vm = Record vm_record
  let make_ref x = make_ref VM vm x
end
...
module VBD = struct
  ...
  let empty_vbd = {vBD_VM=(NullRef VM)}
  let vbd_record = let fields = [B vM] in {r_fields = (fun _ -> fields); r_empty = empty_vbd}
  let vbd = Record vbd_record
  let make_ref x = make_ref VBD vbd x
end
...
module DB = struct
  ...
  let db_record = let fields = [ B vms; B vbds ] in {r_fields = (fun _ -> fields); r_empty = {vms=RefMap.empty; vbds=RefMap.empty}}
end
...
let rec unmarshal : type a. a typ -> a option -> Rpc.t -> a = fun ty cur rpc ->
  match ty,rpc with
  | Dummy, _ -> failwith "Can't unmarshal a dummy value"
